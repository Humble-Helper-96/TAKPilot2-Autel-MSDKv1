...
class ArOverlayView @JvmOverloads constructor(
    ...
    /**
     * Video image bounds within this view. On the DJI blueprint this is fed from
     * `FpvTextureView.onVideoRectChanged` (its video is letterboxed inside the view); Autel's
     * `AutelCodecView` is an opaque SDK widget with no content-rect callback, so this defaults
     * to the full view bounds — the SAME assumption [CrosshairView] documents, and the two MUST
     * share it: a freshly dropped pin has to render under the crosshair, which only holds if
     * overlay and crosshair agree where the image centre is. If Phase 4 finds internal
     * letterboxing, feed both via [setVideoRect].
     */
    ...
    private fun drawContacts(
        ...
        for (u in users) {
            ...
            val targetGroundMsl = DtedIndex.elevationAt(context, lat, lon)
    ...
    private fun project(dBearingDeg: Double, dElevDeg: Double): Pair<Float, Float>? {
        ...
        val halfH = Math.toRadians(AutelTakBridge.hFovDeg(zoom) / 2.0)
    ...
    private fun drawPin(canvas: Canvas, x: Float, y: Float, pin: TakDropMarkers.PinInfo) {
        ...
        val bmp = iconCache.getOrPut(pin.affiliation.res) {
            TakMapMarkers.drawableToBitmap(context, pin.affiliation.res, size)
                ?: run {
    ...
    companion object {
         ...
         *  now," the same way [VideoStreamerHolder]'s state is queried
